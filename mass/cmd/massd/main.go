...
import (
	...
	"github.com/argpass/golab/mass/internal/constant"
	"github.com/argpass/golab/mass/internal/libs/utils"
...
func (p *program) Run(ctx context.Context) error {
	logger := ctx.Value(constant.CtxKeyOfLogger).(*zap.Logger)
	...
	if p.RawConfig.HasField("massd") {
		harvesterdConfig, err = p.RawConfig.Child("massd", -1)
		if err != nil {
...
func main() {
	...
	ctx, cancel := context.WithCancel(
		context.WithValue(
			context.Background(), constant.CtxKeyOfLogger, logger))
